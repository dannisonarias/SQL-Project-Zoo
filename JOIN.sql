...
-- You can combine
-- the two steps into a single query
-- with a JOIN.

-- SELECT *
-- FROM game JOIN goal ON (id=matchid)

-- The FROM clause says to
-- merge data from the goal table with that from the game table. The ON says how to figure out which rows in game go
...
-- The code below shows the player
-- (from the goal) and stadium name
...
-- Use the
-- same JOIN as in the previous question.
...
-- List the player
-- for every goal scored in a game where the stadium was 'National Stadium, Warsaw'
...
-- The example queryshows all goals scored in the Germany-Greece quarterfinal.
...
-- Show teamname and the total number of goals scored. COUNT and GROUP BY
